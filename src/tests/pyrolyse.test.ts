...
import { TIER_HV, TIER_UHV, TIER_ZPM } from '../utils';
...
describe('Pyrolyse Oven', () => {
    ...
    it('caps TDA overclocks at the 1 tick minimum duration', () => {
        const model = solveTda(TIER_UHV);
        const recipe = Repository.current.GetById<Recipe>(CHARCOAL_BYPRODUCTS)!;
        const byproducts = recipe.items.find((slot) =>
            slot.type === RecipeIoType.FluidOutput && slot.goods.id === 'f:gtceu:charcoal_byproducts')!;

        expect(model.overclockTiers).toBe(6);
        expect(model.overclockName).toBe('OC x6');
        expect(model.recipesPerMinute).toBeCloseTo(4800, 5);
        expect(model.flow.output[byproducts.goods.id] / 60).toBeCloseTo(320000, 5);
    });

    it('matches the TDA charcoal-byproducts batch rate with Naquadah coils at HV', () => {
        const model = solveTda(TIER_HV);
        const recipe = Repository.current.GetById<Recipe>(CHARCOAL_BYPRODUCTS)!;
        const byproducts = recipe.items.find((slot) =>
            slot.type === RecipeIoType.FluidOutput && slot.goods.id === 'f:gtceu:charcoal_byproducts')!;
